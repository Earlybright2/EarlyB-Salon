...
import {
  Users, ShoppingBag, Calendar, TrendingUp, DollarSign,
  Star, ShieldCheck, AlertTriangle, ChevronRight, BarChart3,
  ArrowUpRight, ArrowDownRight, Activity, UserCheck, Scissors,
  Package, CreditCard, FileText, Flag, MessageSquare,
  Settings, Image as ImageIcon, Sparkles, Ban, CheckCircle2,
  XCircle, Clock, Search, Eye, LogOut, Menu, LayoutDashboard,
} from "lucide-react";
...
export default function Admin() {
  ...
  const dashboard = trpc.admin.dashboard.useQuery(undefined, { enabled: isAuthenticated && allowedTabs.includes("overview") });
  const platformStats = trpc.admin.platformStats.useQuery(undefined, { enabled: isAuthenticated && allowedTabs.includes("overview") });
  const topSalons = trpc.admin.topSalons.useQuery(undefined, { enabled: isAuthenticated && allowedTabs.includes("overview") });
    ...
    <div className="min-h-screen bg-ebs-bg pt-20">
      <div className="flex">
        ...
        <main className="flex-1 lg:ml-64 p-6 lg:p-10">
          ...
          {/* ─── OVERVIEW ─────────────────────────────── */}
          {activeTab === "overview" && <OverviewTab dashboard={dashboard.data} platformStats={platformStats.data} topSalons={topSalons.data} role={currentRole!} />}
...
// ─── OVERVIEW TAB ─────────────────────────────────────
function OverviewTab({ dashboard, platformStats, topSalons, role }: {
  dashboard: any; platformStats: any; topSalons: any; role: AdminRole;
}) {
  return (
    <>
...
function UsersTab() {
  ...
  const updateRole = trpc.admin.updateUserRole.useMutation({ onSuccess: () => utils.admin.allUsers.invalidate() });
  const suspendUser = trpc.admin.suspendUser.useMutation({ onSuccess: () => utils.admin.allUsers.invalidate() });
...
function StylistsTab() {
  const { data: stylistsData, isLoading } = trpc.admin.allStylists.useQuery();
  return (
    <div className="rounded-2xl bg-ebs-bg-card border border-white/5 overflow-hidden">
...
// ─── SALONS TAB ───────────────────────────────────────
function SalonsTab() {
  const { data: salonsData } = trpc.shop.salons.useQuery();
  return (
    <div className="rounded-2xl bg-ebs-bg-card border border-white/5 overflow-hidden">
...
function ProductsTab() {
  const { data: productsData } = trpc.shop.products.useQuery();
  return (
    <div className="rounded-2xl bg-ebs-bg-card border border-white/5 overflow-hidden">
      <div className="p-6 border-b border-white/5 flex items-center justify-between">
...
// ─── KYC TAB ──────────────────────────────────────────
function KycTab() {
  const { data: pendingKyc, isLoading } = trpc.admin.pendingKycList.useQuery();
  const utils = trpc.useUtils();
  const approveKyc = trpc.admin.approveKyc.useMutation({ onSuccess: () => utils.admin.pendingKycList.invalidate() });
...
function DisputesTab() {
  const { data: disputes, isLoading } = trpc.admin.pendingDisputes.useQuery();
  return (
    <div className="rounded-2xl bg-ebs-bg-card border border-white/5 overflow-hidden">
      <div className="p-6 border-b border-white/5">
...
// ─── REVIEWS TAB ──────────────────────────────────────
function ReviewsTab() {
  const { data: flaggedReviews, isLoading } = trpc.admin.flaggedReviews.useQuery();
  const utils = trpc.useUtils();
  const moderateReview = trpc.admin.moderateReview.useMutation({ onSuccess: () => utils.admin.flaggedReviews.invalidate() });
...
function AnalyticsTab() {
  const { data: platformStats } = trpc.admin.platformStats.useQuery();
  return (
